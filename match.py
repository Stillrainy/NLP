...
def sentence_similarity(str1 = None, str2 = None):
    ...
    word1=sm.reduce(word1)  #提取原句关键词
#    word2=sm.reduce(word2)

    print(word1['cws'],'\n',word1['pos'],'\n')
    print(word2['cws'],'\n',word2['pos'],'\n')

    def max_similarity(word1, word2):
        total = 0; match = len(word1['cws'])
        for i in range(len(word1['cws'])):
            max = 0; word2_id = 0; max_id = -1
            for j in range(len(word2['cws'])):
                if(word2['use'][word2_id] == False):
                    tmp = word_similarity(word1['cws'][i],word2['cws'][j])['data']['similarity']
...
def section_similarity(str1 = None, str2 = None):
    ...
    while '' in rs2:
        rs2.remove('')
    sentence1 = {}; sentence2 ={}
    sentence1['word']=[]; sentence1['ues']=[]
    sentence2['word']=[]; sentence2['ues']=[]
    for s in rs1:
    ...
    total = 0; match = len(sentence1['word'])
    for s2 in sentence2['word']:
        max = 0; tmp_id = 0; max_id = 0
